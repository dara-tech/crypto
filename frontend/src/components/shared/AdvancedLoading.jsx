...
const AdvancedLoading = ({ type = 'default' }) => {
  const renderHeroSkeleton = () => (
    <div className="relative h-[600px] w-full overflow-hidden bg-base-200">
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <div className="w-28 h-28 rounded-full bg-base-300 animate-pulse mb-6"></div>
        <div className="h-12 w-96 max-w-full bg-base-300 rounded-lg animate-pulse mb-4"></div>
        <div className="h-6 w-[500px] max-w-full bg-base-300 rounded animate-pulse mb-2"></div>
        <div className="h-6 w-[400px] max-w-full bg-base-300 rounded animate-pulse"></div>
      </div>
    </div>
  );
        ...
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="card bg-base-100 shadow-xl">
            <div className="card-body items-center text-center">
              <div className="w-12 h-12 rounded-full bg-base-300 animate-pulse mb-2"></div>
              <div className="h-8 w-24 bg-base-300 rounded animate-pulse mb-2"></div>
  ...
  const renderCardsSkeleton = () => (
    <div className="container mx-auto px-4 py-16">
      ...
      >
        <div className="h-12 w-64 bg-base-300 rounded animate-pulse mx-auto"></div>
      </motion.h2>
    ...
    <div className="overflow-x-auto">
      <table className="table w-full">
        <thead>
          <tr>
            {[1, 2, 3, 4, 5].map((i) => (
              <th key={i} className="h-12 bg-base-300 animate-pulse"></th>
            ))}
          </tr>
        </thead>
        <tbody>
          {[1, 2, 3, 4, 5].map((row) => (
            <tr key={row}>
              {[1, 2, 3, 4, 5].map((col) => (
                <td key={col} className="h-16 bg-base-300 animate-pulse"></td>
              ))}
            </tr>
          ))}
        </tbody>
